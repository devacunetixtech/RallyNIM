...
export const config = {
  ...
  mongoUri: isProduction ? requiredEnv('MONGODB_URI') : (process.env.MONGODB_URI),
  jwtSecret: isProduction ? requiredEnv('JWT_SECRET') : (process.env.JWT_SECRET),
  ...
  escrowWalletAddress: isProduction ? requiredEnv('ESCROW_WALLET_ADDRESS') : (process.env.ESCROW_WALLET_ADDRESS),
